...
class Github{
    ...
    */
    async cloneRepository() {
        const destinationPath = `${process.env.STORAGE_PATH}/containers/${this.user._id}/github-repos/${this.repository._id}`;
        try {
    ...
    */
    async readEnvironmentVariables(){
        let envFiles = await simpleGit(`${process.env.STORAGE_PATH}/containers/${this.user._id}/github-repos/${this.repository._id}`).raw(['ls-tree', 'HEAD', '-r', '--name-only']);
        envFiles = envFiles.split('\n').filter(file => file.includes('.env'));
        const environmentVariables = {};
        for(const envFile of envFiles){
            const file = await simpleGit(`${process.env.STORAGE_PATH}/containers/${this.user._id}/github-repos/${this.repository._id}`).raw(['show', 'HEAD:' + envFile]);
