...
exports.webhook = catchAsync(async (req, res) => {
    ...
    if(!requestedRepository) throw new RuntimeError('Repository::Not::Found');
    ...
    await User.updateOneById(repositoryUser._id, {
        $push: { deployments: deployment._id }
    });
    await Repository.updateOneById(requestedRepository._id, {
        $push: { deployments: deployment._id }
    });
    res.status(200).json({ status: 'success' });
});